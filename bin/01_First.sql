...
-- Create : 테이블, 뷰, 사용자 등을 생성할때 사용하는 명령
-- Alter : 이미 생성되어 있는 테이블 또는 뷰, 사용자 등의 구조를 수정하기 위한 명령
-- Drop : 이미 생성되어 있는 데이블 또는 뷰, 사용자 등을 삭제하기 위한 명령


-- DML(Data  Managment Language)- 데이터 조작어(레코드 단위의 처리)
...
-- 기본키(Primary Key) : 테이블을 구성하는 필드들 중에서  갖는 값들이 빈칸(null)이 없고 서로 다른 
--              값(유일한값)들을 갖고 있어 레코드들을 유일하게 구분해 낼 수 있는 필드
--              테이블을 구성하는 필드들중 자격이되는 필드중 하나에 부여하는 테이블의 대표값
--   기본키를 지정함으로써 기본키가 지정하지 않았을때 발생할수 있는 오류들을 미연에 방지할 수 있다.
--   기본키로 결함을 없어지게해서 결함없이 유지되는 것을 "개체 무결성" 이라고 부릅니다
